...
// ── PROXY POOL ─────────────────────────────────────────────────────────────────
let proxyList = [];   // Array of proxy URL strings
...
function parseProxy(raw) {
    ...
    if (/^(http|https|socks4|socks5):\/\//i.test(raw)) return raw;
    // ip:port:user:pass
    const parts = raw.split(':');
    if (parts.length === 4) {
        return `http://${parts[2]}:${parts[3]}@${parts[0]}:${parts[1]}`;
    }
    // user:pass@ip:port — leave as-is with http://
...
function buildKahootClient() {
    const proxyUrl = nextProxy();
    if (!proxyUrl) return new Kahoot();
    const agent = getProxyAgent(proxyUrl);
    return new Kahoot({
        proxy: (opts) => { opts.agent = agent; return opts; },
        wsproxy: (url) => ({ address: url, agent })
    });
}
...
// ── CHESS GAMES (server-side move relay) ──────────────────────────────────────
const chessGames = new Map(); // gameId -> { white, black, moves[], started }
const CHESS_GAME_TTL = 6 * 60 * 60 * 1000; // 6 hours

setInterval(() => {
    const cutoff = Date.now() - CHESS_GAME_TTL;
    for (const [id, g] of chessGames.entries()) {
        if (g.started < cutoff) chessGames.delete(id);
    }
}, 30 * 60 * 1000);
...
app.get('/chess/challenges', (req, res) => {
    ...
    for (const c of chessChallenges.values()) {
        ...
        if (c.from === me) outgoing.push({ id: c.id, to: c.toDisplay, status: c.status, gameId: c.gameId || null });
    }
...
app.post('/chess/challenge/respond', (req, res) => {
    ...
    if (accept) {
        ...
        chessGames.set(gameId, {
            white: c.from,   // challenger plays white
            black: c.to,     // challenged plays black
            moves: [],
            started: Date.now()
        });
        c.gameId = gameId;
    }
    setTimeout(() => chessChallenges.delete(id), 30000);
    res.json({ success: true, gameId });
});

// ── CHESS GAME STATE ──────────────────────────────────────────────────────────
app.get('/chess/game/:id', (req, res) => {
    const s = getSession(req);
    if (!s) return res.status(401).json({ error: 'Not logged in' });
    const game = chessGames.get(req.params.id);
    if (!game) return res.json({ error: 'Game not found' });
    res.json({ moves: game.moves, white: game.white, black: game.black });
});

app.post('/chess/game/:id/move', (req, res) => {
    const s = getSession(req);
    if (!s) return res.status(401).json({ error: 'Not logged in' });
    const game = chessGames.get(req.params.id);
    if (!game) return res.json({ success: false, error: 'Game not found' });
    const { from, to, flags, promo, color } = req.body;
    if (typeof from !== 'number' || typeof to !== 'number') {
        return res.json({ success: false, error: 'Invalid move data.' });
    }
    game.moves.push({ from, to, flags: flags || 0, promo: promo || null, color, at: Date.now() });
    res.json({ success: true, moveIndex: game.moves.length - 1 });
});
...
app.post('/friends/add', (req, res) => {
    ...
    // Mutual: add me to their friends too
    const them = users.get(themKey);
    if (them) {
        if (!them.friends) them.friends = [];
        if (!them.friends.map(f => f.toLowerCase()).includes(meKey)) {
            them.friends.push(s.username);
            users.set(themKey, them);
        }
    }
    saveUsers(users);
    res.json({ success: true });
...
app.post('/proxy/set', (req, res) => {
    const session = getSession(req);
    if (!session) return res.status(401).json({ error: 'Not logged in' });
    const { proxies } = req.body;
    if (!Array.isArray(proxies) && typeof proxies !== 'string') {
        return res.json({ success: false, error: 'Expected a proxies array or newline-separated string.' });
    }
    const lines = Array.isArray(proxies) ? proxies : proxies.split('\n');
...
app.get('/proxy/list', (req, res) => {
    const session = getSession(req);
    if (!session) return res.status(401).json({ error: 'Not logged in' });
    res.json({ proxies: proxyList, count: proxyList.length });
});

app.post('/proxy/clear', (req, res) => {
    const session = getSession(req);
    if (!session) return res.status(401).json({ error: 'Not logged in' });
    proxyList = [];
    proxyIndex = 0;
...
app.get('/debug-three', (req, res) => {
    const fs2 = require('fs');
    const p = require('path');
    const f = p.join(__dirname, 'node_modules/three/build/three.module.js');
    const exists = fs2.existsSync(f);
    res.json({ exists, path: f, cwd: __dirname });
});
...
function sseRoute(clientList) {
    return (req, res) => {
        ...
        res.flushHeaders();
        // 2KB padding comment flushes buffering proxies immediately
        res.write(SSE_PADDING);
        // Immediate ping so the client knows the stream is live
        res.write(': ping\n\n');
        clientList.push(res);
...
app.post('/start', (req, res) => {
    ...
    function spawnBot(index) {
        if (floodStopped || floodGen !== myGen) return;
        const name   = getBotName(index);
        const client = buildKahootClient();
        client.setMaxListeners(Infinity);

        client.join(pin, name).catch(err => {
            const msg = (err && (err.description || err.message)) || String(err);
            if (msg.toLowerCase().includes('duplicate') && useRandom && !floodStopped && floodGen === myGen) {
        ...
        client.on('Disconnect', reason => {
            if (reason !== 'Quiz Locked' && !floodStopped && floodGen === myGen) setTimeout(() => spawnBot(index), 1000);
        });
    }
...
app.post('/spam-start', (req, res) => {
    ...
    function spawnSpammer(index) {
        if (spamStopped || spamGen !== myGen) return;
        ...
        client.on('Disconnect', reason => {
            if (reason !== 'Quiz Locked' && !spamStopped && spamGen === myGen) setTimeout(() => spawnSpammer(index), 1000);
        });
    }
...
app.post('/dominate', (req, res) => {
    ...
    function spawnDominator(index) {
        if (dominateStopped || dominateGen !== myGen) return;
        ...
        client.on('Disconnect', reason => {
            if (reason !== 'Quiz Locked' && !dominateStopped && dominateGen === myGen) setTimeout(() => spawnDominator(index), 1000);
        });
    }
