...
function getSession(req) {
    ...
    const cm = cookie.match(/(?:^|;\s*)session=([^;]+)/);
    if (cm) candidates.push(cm[1]);
...
const ADMIN_PASS_HASH = hashPassword('P@ssword12345');
...
app.post('/register', (req, res) => {
    ...
    if (username.toLowerCase() === ADMIN_USER) return res.json({ success: false, error: 'Username already taken.' });
    if (users.has(username.toLowerCase())) return res.json({ success: false, error: 'Username already taken.' });
    users.set(username.toLowerCase(), { username, password: hashPassword(password), starcoins: 0 });
    saveUsers(users);
...
app.post('/auth', (req, res) => {
    ...
    if (username.toLowerCase() === ADMIN_USER) {
        ...
        if (!users.has(ADMIN_USER)) {
            users.set(ADMIN_USER, { username: ADMIN_USER, starcoins: 0 });
            saveUsers(users);
        }
...
app.get('/api/starcoins', (req, res) => {
    const s = getSession(req);
    if (!s) return res.status(401).json({ error: 'Not logged in' });
    const user = users.get(s.username.toLowerCase());
    res.json({ starcoins: user ? (user.starcoins || 0) : 0 });
});

app.post('/admin/set-coins', (req, res) => {
    const s = getSession(req);
    if (!s || s.username !== '293802') return res.status(403).json({ error: 'Forbidden' });
    const { username, amount } = req.body;
    const key = username.toLowerCase();
    const user = users.get(key);
    if (!user) return res.json({ success: false, error: 'User not found.' });
    user.starcoins = Math.max(0, parseInt(amount) || 0);
    users.set(key, user);
    saveUsers(users);
...
app.get('/admin/users', (req, res) => {
    ...
    for (const [key, val] of users.entries()) {
        list.push({ username: val.username, starcoins: val.starcoins || 0 });
    }
...
app.post('/admin/delete-user', (req, res) => {
    ...
    if (!username || username === '293802') return res.json({ success: false, error: 'Cannot delete this user.' });
    const key = username.toLowerCase();
